...
const DRIVER_NAME = 'githubdeluxe'
...
/**
 * This doesn't work because files doesn't persist
 * between build and generate
 */
let files = {}
export function prerenderRoutes() {
  return Object.keys(files).map(file => file.replace('.md',''))
}


/**
 * This doesn't work because module state isn't saved 
 * between build and generate
 *  
export async function prerenderRoutes() {
  console.log(opts)
  return Object
    .keys(await fetchFiles(opts))
...
export default defineDriver((_opts) => {
  ...
  const syncFiles = async () => {
    ...
    console.log('syncFiles - complete', Object.keys(files))
  }
